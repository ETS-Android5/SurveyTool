...
var Offlinepagechange = function (formId, n) {
    this.formid = formId;
    this.totalpages = n;
    this.current = 1;
    var form = jQuery('#gform_wrapper_' + formId),
        progressbar = jQuery(".gf_progressbar_percentage", form),
        progresstext = jQuery("span", progressbar);
    this.form = form;
    this.progressbar = progressbar;
    this.progresstext = progresstext;
}
Offlinepagechange.prototype = {
    init: function () {
        var d = this,
            el = d.form;
        el.data('offlinepagechanger', d);
    }, nextp: function () {
        var d = this;
        if (d.totalpages > d.current) {
            d.current++;
        }
        d.loop_display();
    }, previousp: function () {
        ...
        var d = this,
            now_percent = d.current / d.totalpages * 100,
            text = Math.floor(now_percent) + "%";
        d.progressbar.css('width', text);
        d.progresstext.html(text);
        console.log(text);
        for (var i = 1; i <= d.totalpages; i++) {
            if (i === d.current) d.getElPage(i, true); else d.getElPage(i, false);
        }
    }, getElPage: function (page_at, show) {
        var id = this.formid,
            pid = '#gform_page_' + id + "_" + page_at,
            el = jQuery(pid);
        if (el.size() > 0) {
            if (show) el.show(); else el.hide();
        }
    }, save: function () {
        var serializedf = this.form.serialize(),
            arrayf = this.form.serializeArray(),
            data = JSON.stringify(arrayf);
        console.log(data);
    }
}

...
/**
 * Created by Hesk on 10/7/2014.
 */
public class PicTransformMegaPixel implements Transformation {
    public String TAG = "mega pixel transform";
    //start with this mega pixel in here
    private int IMAGE_MAX_SIZE = 100000;

    public PicTransformMegaPixel(int MaxMP) {
        this.IMAGE_MAX_SIZE = MaxMP;
    }

    @Override
    public Bitmap transform(Bitmap source) {
        int scale = 1;
        while ((source.getWidth() * source.getHeight()) * (1 / Math.pow(scale, 2)) > IMAGE_MAX_SIZE) {
            scale++;
        }
        if (scale > 1) {
            scale--;

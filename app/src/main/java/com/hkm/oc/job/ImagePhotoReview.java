...
/**
 * Created by Hesk on 15/7/2014.
 */
public class ImagePhotoReview extends BaseLayerAppearance {
    private ImageView mImageView;
    private PhotoViewAttacher mAttacher;
    private ImageButton btn;

    private int position, attachment_id;
    private TextView mtextView;

    private Uri display_url;
    ...
    protected void postOnCreate() {
        ...
        display_url = Uri.parse(current_job_task.get_list_location_map().get(position).get_url());
        attachment_id = current_job_task.get_list_location_map().get(position).get_attachment_id();
        ...
        Picasso.with(this).load(display_url).centerInside().fit().placeholder(R.drawable.alpha_1callpower).error(R.drawable.ic_alerts_and_states_error_big).into(mImageView);

        btn = (ImageButton) findViewById(R.id.bn_change_description);
        btn.setVisibility(View.GONE);
        mtextView = (TextView) findViewById(R.id.photo_view_description);
        mtextView.setText("Attachment ID: " + attachment_id);
    }

    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.image_retrieve_menu, menu);
        menu.removeItem(R.id.edit_tag);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.edit_label:
                use_as_draw_map_start();
                break;
            case R.id.edit_tag:
                break;
            case android.R.id.home:
                // getActionBar().setHomeButtonEnabled(false);
                setResult(Constant.IntentAction.UPDATE);
                break;
            //return true;
        }
    ...
    private void use_as_draw_map_start() {
        try {
            if (current_job_task.hasSketchMapDrawn(attachment_id)) {
                final DialogSimpleNotification dialog = DialogSimpleNotification.newInstance(R.string.notice_already_have_draw_map);
                dialog.show(getSupportFragmentManager(), "alreadystarted"); //with this tag name
            } else {
                final DialogDoYouWantTo dialog = DialogDoYouWantTo.newInstance(R.string.question_start_map_draw, new DCBBool() {
                    @Override
                    public void onyes(DialogInterface dialog) {
                        start_plan(current_job_task.get_list_location_map().get(position));
                    }
                ...
                dialog.show(getSupportFragmentManager(), "question"); //with this tag name
            }
        } catch (Exception e) {

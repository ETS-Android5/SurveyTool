...
public class SlidingPanel extends ViewGroup {
    ...
    /**
     * The default max height ratio is set to 2/3rd of the screen height when the panel is drawn out
     */
    ...
    public int getTopDistance(int heightSize) {
        ...
        Log.d("get meansured height", "" + maxContentHeight);
        ...
        Log.d("get meansured topValue", "" + topValue);
        return topValue;
    }
...
/** it cannot carry one single child because it does not take that */
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child instanceof SurfaceView) {
               // ((SurfaceView) child).getHolder().setFixedSize(childWidthSpec, childHeightSpec);
    ...
    public boolean expand() {
        if (mFirstLayout || smoothSlideTo(0.0f)) {
    ...
    public boolean collapse() {
        if (mFirstLayout || smoothSlideTo(1.0f)) {
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            return !mIsUnableToDrag && ((SlidingPanel.LayoutParams) child.getLayoutParams()).slideable;
        }

...
/**
 * Created by Hesk on 4/7/2014.
 */
public class upload_verification extends AsyncTask<Void, Void, String> {
    ...
    public upload_verification(completecb cb, appWork ac) {
        this.cb = cb;
        this.ac = ac;
    }
    ...
    protected void onPostExecute(String result) {
        if (result.equalsIgnoreCase("done")) {
            ...
            sb.append(ac.getResources().getString(R.string.notice_check_submission_result));
            sb.append("\n");
            if (!check_maps) {
                sb.append(ac.getResources().getString(R.string.verification_condition_1));
                sb.append("\n");
            }
            if (!check_photos) {
                sb.append(ac.getResources().getString(R.string.verification_condition_2));
                sb.append("\n");
            }
           /* if (!current_job_task.working_customer_survey_complete)
                result += "- Customer survey form is not completed.\n";*/
            if (!current_job_task.working_form_line_record_complete) {
                sb.append(ac.getResources().getString(R.string.verification_condition_3));
                sb.append("\n");
            }
            if (!check_gps) {
                sb.append(ac.getResources().getString(R.string.verification_condition_4));
            }
            cb.cbfailure(sb.toString());
        }
    }
    ...
    protected String doInBackground(Void... strings) {
        ...
        // ArrayList<LocationMap> maps = current_job_task.get_list_location_map();
        ArrayList<SketchMapData> dmap = current_job_task.getSketchMapList();
        ArrayList<PhotoViewData> hmap = current_job_task.getPhotoList();
        current_job_task.set_android_id(Tool.getDeviceId(ac));

        int p = 0, pt = dmap.size();
        ...
        for (PhotoViewData d : hmap) {
            if (d.tag.equalsIgnoreCase("Location Photo")) {
                p++;
            }
        }
        check_gps = current_job_task.GPS_ready();
        // check_maps = lt > 0 ? l == lt : false;
        check_photos = hmap.size() > 0 && p > 2;
        // publishProgress(2);
        boolean all_done = check_gps && check_maps && check_photos && current_job_task.working_form_line_record_complete;
        if (ac.IsDebug()) all_done = true;
        return all_done ? "done" : "fail";
    }
}

 ...
 * License Grant: Licensor hereby grants Licensee a Sublicensable, Assignable, Commercial with terms, Including the rights to create but not distribute derivative works, Non-exclusive license, all with accordance with the terms set forth and other legal restrictions set forth in 3rd party software used while running Software.
 ...
 * Commercial use allowed with restrictions: by agreements and terms granted by the author.
 ...
 * If Software is covered by Patents, Licensor grants Licensee a non-exclusive, worldwide, royalty-free patent license under Licensor's essential patent claims, to make, use, sell, offer for sale, import and otherwise run, modify and propagate the contents of Software.
...
package com.hkm.oc.wv.supportwebview;
...
/**
 * Created by Hesk on 7/7/2014.
 * name: onecall_data_support
 */
public class JSInterfaceSupport {
    protected static appWork ac;
    protected webviewSupports ctx;

    public JSInterfaceSupport(webviewSupports m, appWork ac) {
        this.ctx = m;
        this.ac = ac;
    }

    private void completeMessageBox(final String saved_data) {
        final DsingleCB handle = new DsingleCB() {
            @Override
        ...
        if (ac.IsDebug()) {
            DialogSimpleNotification d = DialogSimpleNotification.newInstance(saved_data, handle);
            d.show(ctx.getSupportFragmentManager(), "save data");
        } else {
            DialogSimpleNotification d = DialogSimpleNotification.newInstance(R.string.notice_form_save_and_complete, handle);
            d.show(ctx.getSupportFragmentManager(), "save data");
        }
    }

    @JavascriptInterface
    public void saveform(final String fromID, final String form_data) {
        Tool.trace(ctx, "save form 2");
        if (fromID.equalsIgnoreCase("gform_16")) {
            current_job_task.saveWLRForm(form_data);
            if (!current_job_task.working_form_line_record_complete) {
                DialogSimpleNotification d = DialogSimpleNotification.newInstance(R.string.notice_sign_now_both_parties, new DsingleCB() {
                    @Override
                    public void oncontified(DialogFragment dialog) {
                        dialog.dismiss();
                    }
                ...
                d.show(ctx.getSupportFragmentManager(), "save data");
            } else completeMessageBox(form_data);
        }
        if (fromID.equalsIgnoreCase("gform_17")) {
            current_job_task.saveWCSForm(form_data);
            if (!current_job_task.working_customer_survey_complete) {
                DialogSimpleNotification d = DialogSimpleNotification.newInstance(R.string.notice_sign_now_only_client, new DsingleCB() {
                    @Override
                    public void oncontified(DialogFragment dialog) {
                        dialog.dismiss();
                    }
    ...
    @JavascriptInterface
    public void checkCompleteReturn(final String fromID, final String resultd, final String sigature_id) {
        int sid = Integer.parseInt(sigature_id);
        int result = Integer.parseInt(resultd);
        if (result == 1) {
            //complete
            ctx.startSign(sid);
        }
        if (result == 0) {
            //incomplete
            DialogSimpleNotification d = DialogSimpleNotification.newInstance(R.string.notice_not_form_not_complete);
            d.show(ctx.getSupportFragmentManager(), "save data");
        }
        //to trigger directly on the html page
    ...
    @JavascriptInterface
    public void clientSignTrigger() {

    }

    @JavascriptInterface
    public void workerSignTrigger() {

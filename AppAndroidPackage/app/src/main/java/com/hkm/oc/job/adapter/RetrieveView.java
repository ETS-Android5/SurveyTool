...
import com.hkm.oc.R;
import com.hkm.oc.job.ImagePhotoReview;
import com.hkm.oc.job.JobTaskActivity;
import com.squareup.picasso.Picasso;

import static com.hkm.U.Constant.IntentKey.OPEN_RETRIEVED_IMAGES;
...
public class RetrieveView {
    ...
    public RetrieveView(Context ctx, View Convertedview, int position) {
        this.ctx = ctx;
  ...
  /*  protected File getImageFile() {
    ...
    public RetrieveView update(int position) {
        this.pos = position;

        this.mUri = Uri.parse(current_job_task.get_list_location_map().get(position).get_url());
        this.mText = current_job_task.get_list_location_map().get(position).get_attachment_id() + "";
        return this;
    }

    public void onEdit() {
        Intent slnt = new Intent(ctx, ImagePhotoReview.class);
        Bundle b = new Bundle();
        b.putInt("position", this.pos);
        slnt.putExtras(b);
        //slnt.putExtra("i_uri", mUri);
        // ctx.startActivity();
        ((JobTaskActivity) ctx).startActivityForResult(slnt, OPEN_RETRIEVED_IMAGES);
    }

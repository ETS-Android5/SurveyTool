...
import com.hkm.oc.panel.corepanel.elements.mathmodels.Route_render_loop;
import com.hkm.oc.panel.corepanel.elements.mathmodels.Route_render_path_labels;
...
public class Route {
    ...
    public boolean check_for_auto_connect_mode() {
        boolean fact1 = Content.current_sketch_map.getRouteNodeList().size() > 0;
        boolean fact2 = containDots.size() == 0;
        if (fact1 && fact2) {
            for (RouteNode rn : Content.current_sketch_map.getRouteNodeList()) {

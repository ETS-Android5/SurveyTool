...
public class DataHandler {
    ...
    private static String[] break_down(int position) {
        return Content.radius_list.get(position).split(",");
    }
    ...
    public static String[] get_previous_labels() {
        final int i = Content.radius_list.size();
        if (i > 0) {
            String[] f = new String[i + 1];
            f[0] = "Replace by ref. point..";
            for (int k = 1; k <= i; k++) {
                final String c = get_string_label_for_drawings(Content.radius_list.get(k - 1));
    ...
    public static boolean result_update_from_the_previous_item(int real_choice, int target_index) {
        if (real_choice >= 0) {
            final String loaded = Content.radius_list.get(real_choice);
            Content.radius_list.set(target_index, loaded);
            return true;
        } else
            return false;
    }

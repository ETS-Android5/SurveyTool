...
//http://tech.chitgoks.com/2012/07/08/android-draw-equilateral-triangle-shapes-in-canvas/
    ...
    public static float angleFromPoint(PointF a, PointF b) {
        final PointF vec = ToolBox.vector(a, b);
    ...
    public static int m2p(float meter) {
        double px = meter * Content.dpi / C.inch_meter_ratio * 1 / Content.current_sketch_map.getratio();
        //double pixel = (meter * 100 * Content.dpi) / (Content.ratio * 25.4f);
        return (int) px;
    }

    public static float p2m(float pixel) {
        //double meter = (pixel * 25.4f * Content.ratio) / (100 * Content.dpi);
        double meter = pixel * Content.current_sketch_map.getratio() * C.inch_meter_ratio / Content.dpi;
        return (float) meter;
    }

...
//http://tech.chitgoks.com/2012/07/08/android-draw-equilateral-triangle-shapes-in-canvas/
    ...
    public static int m2p(float meter) {
        double px = meter * AppDataRetain.dpi / inch_meter_ratio * 1 / AppDataRetain.current_sketch_map.getratio();
        //double pixel = (meter * 100 * AppDataRetain.dpi) / (AppDataRetain.ratio * 25.4f);
        return (int) px;
    }

    public static float p2m(float pixel) {
        //double meter = (pixel * 25.4f * AppDataRetain.ratio) / (100 * AppDataRetain.dpi);
        double meter = pixel * AppDataRetain.current_sketch_map.getratio() * inch_meter_ratio / AppDataRetain.dpi;
        return (float) meter;
    }

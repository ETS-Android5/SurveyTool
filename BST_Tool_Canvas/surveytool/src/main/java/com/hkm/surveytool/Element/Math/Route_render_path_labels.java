...
import com.hkm.surveytool.Element.DataPoints.Label;
import com.hkm.surveytool.Element.DataPoints.LabelRenderObject;
...
public class Route_render_path_labels extends Element {
    ...
    public Route_render_path_labels(Canvas setCanvas, Context ctx, ArrayList<Paths> bp, ArrayList<Dot> list) {
        ...
        if (build_paths.size() > 0) {
            AppDataRetain.label_list.clear();
            ...
            if (advancing < length) {
                while (running_distance < length) {
                    ...
                    if (lookup_point_in_zone(dposition)) {
                        ...
                        AppDataRetain.label_list.add(new LabelRenderObject(mlabel, label_position_matrix));
                    }
    ...
    private void render_label_path() {
        if (render_dot_ready) {
            for (LabelRenderObject lb : AppDataRetain.label_list) {
